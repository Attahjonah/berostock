...
// ✅ Add `userRole` param
module.exports = async function generateSummaryPDF(sales, title, startDate, endDate, res, userRole) {
  ...
  doc.text('Amount (NGN)', 350, y, { width: 90, align: 'right' });

  if (userRole === 'admin' || userRole === 'manager') {
    doc.text('Profit (NGN)', 450, y, { width: 90, align: 'right' });
  }
  ...
  for (const sale of sales) {
    ...
    if (userRole === 'admin' || userRole === 'manager') {
      doc.text(`${profit.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 450, y, { width: 90, align: 'right' });
    }
